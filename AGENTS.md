...
- **`core/structure_utils.py`** (new) — pure swing/structure functions with
  ...
  nearest level's role). Split out from `StructureEngine.py`
  ...
  swing functions. (This module originally also held `detect_snd()`, a
  2-candle supply/demand heuristic — removed once `StructureEngine`'s
  `context_zone`/`context_level` moved to sourcing exclusively from
  `core/demand_engine.py`'s `DemandEngine.get_context()` instead.)
